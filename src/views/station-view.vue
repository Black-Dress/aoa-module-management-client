              ...
              <div style="padding: 10px">
                <el-row justify="space-between">
                  <el-col :span="10" style="text-align: left">
                    <el-button text @click="toStationDetil(col.id, i, j)" style="padding: 0 0 0 0">
                      <h1>{{ col.name }}</h1>
                    </el-button>
                  </el-col>
                  <el-col :span="3">
...
export default {
  ...
  methods: {
    ...
    toStationDetil(id, i, j) {
      let index = (this.current_page - 1) * this.page_size + i * this.col_size + j;
      this.$router.push({ name: "station-details", query: { id: id, index: index } });
    },
    remove(i, j) {
